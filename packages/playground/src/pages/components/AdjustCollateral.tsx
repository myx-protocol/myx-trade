  ...
  const handleAdjustCollateral = async () => {
    ...
    try {
      if (myxClient) {
        ...
        const adjustAmount = ethers
          .parseUnits(amount, pool?.quoteDecimals)
          .toString();
        ...
        const result = await myxClient.position.adjustCollateral({
          ...
          poolOracleType: pool?.oracleType ?? OracleType.Chainlink,
        });
        if (result?.code === 0) {
          ...
          message.error(
            `保证金调整失败 / Collateral adjustment failed: ${result?.message}`
          );
        }
      }
    } catch (error) {
            ...
            <Space direction="vertical" style={{ width: "100%" }}>
              ...
              <div style={{ marginTop: 8 }}>
                <Text type="secondary" style={{ fontSize: "12px" }}>
                  提示：输入正数增加保证金，输入负数减少保证金 / Tip: Enter
                  positive number to increase, negative to decrease collateral
                </Text>
              </div>
            </Space>
          </div>
