...
var defaultChains = [arbitrumSepolia];
function getChainInfo(chainId3) {
  return defaultChains.find((chain) => chain.id === chainId3);
}
...
var RotationProvider = class extends import_providers.BaseProvider {
  constructor(urls, chainId3, config) {
    ...
    this.retries = 0;
    this.lastError = "";
    this.providers = urls.map((url) => new import_providers.StaticJsonRpcProvider(url, chainId3));
...
var getJSONProvider = (chainId3) => {
  ...
  if (chainProviders.length === 1) {
    return new import_ethers3.JsonRpcProvider(chainProviders[0], chainId3, {
      staticNetwork: true
    });
  } else {
...
var getWalletProvider = async (chainId3) => {
  try {
    ...
    const provider = new import_ethers3.ethers.BrowserProvider(window.ethereum);
    if (chainId3) {
...
async function getErrorTextFormError(error) {
  ...
  if (decodeErrorResult.type === import_ethers_decode_error.ErrorType.UserRejectError) {
    return {
      error: Errors[4001 /* USER_REJECTED_REQUEST */]
    };
  }
...
var withdraw = async (params) => {
  try {
    ...
    const amountIn = (0, import_ethers9.parseUnits)(amount.toString(), decimals);
