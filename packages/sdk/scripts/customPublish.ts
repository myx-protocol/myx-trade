...
loadingAdapter('检查当前分支', () => checkBranch())
    .then(() => loadingAdapter('检查远程仓库状态', () => checkGitRemote())
    )
    ...
    .then(async (answers) => {
        ...
        const isLoggedIn = await loadingAdapter('检查 npm 登录状态', () => checkNPMLogined())
        if (!isLoggedIn) {
